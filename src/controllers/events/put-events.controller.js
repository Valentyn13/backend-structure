...
const putEventController = async(req, res) => {
    try {
        ...
        const event = await eventService.updateEvent(eventId,{ score: req.body.score })
            ...
            bets.map((bet) => {
              if (bet.prediction == result) {
                ...
                betService.getUserByBetUserId(bet.user_id).then(([user]) => {
                    return betService.updateUserAfterBet(bet.user_id,{balance:user.balance + bet.bet_amount * bet.multiplier}) 
                  });
